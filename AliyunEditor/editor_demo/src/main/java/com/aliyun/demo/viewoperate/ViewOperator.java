package com.aliyun.demo.viewoperate;
...
public class ViewOperator {
    ...
    //int playerBtn
    public ViewOperator(RelativeLayout rootView, ViewGroup titleView, View playerView,View bottomMenuView,View pasterContainerView,View playerBtn) {
    ...
    public void showBottomView(BaseChooser bottomView){
        ...
        if (lp instanceof ViewGroup.MarginLayoutParams){
            ...
            playBtnMarginBottom = rootView.getContext().getResources().getDimensionPixelSize(R.dimen.alivc_svideo_btn_player_margin_b);
        }
        ...
        moveLenth = Math.abs(DensityUtil.dip2px(rootView.getContext(),10 )-playViewMarginTop);
        //计算播放按钮移动距离
        btnTranslationY = playBtnMarginBottom-DensityUtil.dip2px(rootView.getContext(),10 )-bottomViewHeight;
        ...
        if (bottomView.isPlayerNeedZoom()){
            //计算缩放比例
            int h =rootViewHeight -bottomViewHeight- DensityUtil.dip2px(rootView.getContext(),20 );
            SCALE_SIZE = (float)h/playerHeight;
            if (SCALE_SIZE>=0.95f){
                SCALE_SIZE=0.95f;
            }
            Log.d(TAG, "h:"+h+"playerHeight"+playerHeight+"SCALE_SIZE"+SCALE_SIZE+"moveLenth"+moveLenth);
            //  2018/8/24 缩放播放UI
            ...
            anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    Log.e(TAG, "height"+params.height+"width"+params.width);
                    ...
                    int marginTop = (int)Math.abs(playViewMarginTop- moveLenth *(1-currentValue)/(1-SCALE_SIZE));

                    marginParams.setMargins(0, marginTop, 0, 0);
                    Log.e(TAG, "height"+marginParams.height+"width"+marginParams.width+"currentValue"+currentValue
                    +"playerWidth"+playerWidth+"playheight"+playerHeight);
        ...
        if (bottomView.isPlayerNeedZoom()){
            ...
            anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    int marginTop = (int)Math.abs(playViewMarginTop- moveLenth *(1-currentValue)/(1-SCALE_SIZE));//(int)Math.abs(playViewMarginTop-moveLenth*(250-currentTime)/250);
                    marginParams.setMargins(0, marginTop, 0, 0);
                    playerView.setLayoutParams(marginParams);
                    pasterContainerView.setLayoutParams(marginParams);

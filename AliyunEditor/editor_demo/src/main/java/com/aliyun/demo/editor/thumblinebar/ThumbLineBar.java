...
/**
 * Created by cross on 2018/8/22. <p>描述:简单的缩略条实现类 不支持Overlay功能
 */
public class ThumbLineBar extends FrameLayout {
    ...
    private void initView() {
        int indicatorWidth = DensityUtil.dip2px(getContext(), 4);
        mIndicatorMargin = DensityUtil.dip2px(getContext(), 6);
     ...
     */
    private void setThumbLinePlayer(AlivcEditView.PlayerListener linePlayer) {
    ...
    public void show() {
        if (mPlayThread != null && mPlayThread.mState != PlayThread.STATE_PLAYING){
            //暂停时显示缩略条没有对齐
            seekTo(mLinePlayer.getCurrDuration(), false);
        }

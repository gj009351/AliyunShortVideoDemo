...
public abstract class BaseScrollPickerView<T> extends View {
    ...
    private void reset() {
        ...
        if (mCenterItemBackground != null) {
            ...
            mCenterItemBackground.setBounds(
                mCenterX + mItemWidth / 2 - dip2px(5),
    ...
    public void autoScrollTo(final int endY, long duration, final Interpolator interpolator, boolean canIntercept) {
        ...
        mAutoScrollAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                rate[0] = animation.getCurrentPlayTime() * 1f / animation.getDuration();
                computeScroll((int)animation.getAnimatedValue(), endY, rate[0]);
            }
        });
        mAutoScrollAnimator.removeAllListeners();
        mAutoScrollAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if(rate[0] <= 1){
                    ...
                    computeScroll((int)valueAnimator.getAnimatedValue(), endY, rate[0]);
                }
                mIsAutoScrolling = false;
                mDisallowTouch = temp;
            }
    ...
    public void setCenterItemBackground(Drawable centerItemBackground) {
        ...
        mCenterItemBackground.setBounds(
            ...
            mCenterX + mItemWidth / 2 + dip2px(7),
            mCenterY + mItemHeight);
        invalidate();
    }
